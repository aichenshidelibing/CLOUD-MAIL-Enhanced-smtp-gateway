...
grep -q -- 'local_server_ipv6s' "$ROOT/install.sh"
grep -q -- 'is_non_public_ipv4' "$ROOT/install.sh"
grep -q -- 'ahostsv6' "$ROOT/install.sh"
grep -q -- 'TCP port 80 is already in use' "$ROOT/install.sh"
grep -q -- 'tee "$certbot_log"' "$ROOT/install.sh"
grep -q -- 'certbot-$(date +%Y%m%d%H%M%S).log' "$ROOT/install.sh"